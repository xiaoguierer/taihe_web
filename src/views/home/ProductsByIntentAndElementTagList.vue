...
export default {
  ...
  watch: {
    ...
    '$route.params': {
      ...
      handler(newParams) {
        ...
        if (newParams.intentId && newParams.elementTagId) {
          this.currentIntentId = newParams.intentId
          this.currentTagId = newParams.elementTagId
          this.loadAllData()
        } else {
  ...
  async mounted() {
    // 初始化时获取路由参数
    const { intentId, elementTagId } = this.$route.params
    console.log('🚀 组件挂载，接收参数:', { intentId, elementTagId })

    if (intentId && elementTagId) {
      this.currentIntentId = intentId
      this.currentTagId = elementTagId
  ...
  methods: {
    ...
    async fetchProductsData() {
      try {
        const elementTagId = this.currentTagId
        console.log("🔍 请求商品数据，elementTagId:", elementTagId)
        console.log("🔍 请求商品数据，IntentID:", this.currentIntentId)
        const response = await fetch(`/api/product-spu/getProductsByIntentAndElementTag/${this.currentIntentId}/${elementTagId}`);
        if (!response.ok) {
